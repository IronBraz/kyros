...
```bash
# Root
curl -sI https://kyros.neuroforge.club/ | grep "HTTP/2 200"

# SPA routes
curl -sI https://kyros.neuroforge.club/waiting-room | grep "HTTP/2 200"
curl -sI https://kyros.neuroforge.club/your-turn | grep "HTTP/2 200"

# Assets
curl -sI https://kyros.neuroforge.club/assets/index-*.js | grep "HTTP/2 200"
```

**2. Test CLI - Admin PWA**:
```bash
# Root
curl -sI https://kyros.neuroforge.club/control-room/ | grep "HTTP/2 200"

# SPA routes
curl -sI https://kyros.neuroforge.club/control-room/queue | grep "HTTP/2 200"
curl -sI https://kyros.neuroforge.club/control-room/settings | grep "HTTP/2 200"

# Assets
curl -sI https://kyros.neuroforge.club/control-room/assets/index-*.js | grep "HTTP/2 200"
```

**3. Test Browser - Client PWA**:
1. Apri `https://kyros.neuroforge.club/`
...
1. Apri `https://kyros.neuroforge.club/control-room/`
...
curl -vI https://kyros.neuroforge.club/ 2>&1 | grep "SSL certificate verify ok"

# Headers sicurezza
curl -I https://kyros.neuroforge.club/ | grep "X-Content-Type-Options"
curl -I https://kyros.neuroforge.club/ | grep "X-Frame-Options"
```

**6. Test Mobile (opzionale ma raccomandato)**:
- Apri da smartphone: `https://kyros.neuroforge.club/`
